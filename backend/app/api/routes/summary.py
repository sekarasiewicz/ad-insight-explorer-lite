...
async def get_word_frequency(
    limit: Optional[int] = Query(None, description="Limit number of posts to analyze"),
    top_words: Optional[int] = Query(50, description="Number of top words to return"),
):
    """
    Get word frequency analysis across all post titles

    Args:
        limit: Optional limit on number of posts to analyze
        top_words: Number of top words to return (default: 50)
    ...
    try:
        logger.info(
            f"Getting word frequency with limit: {limit}, top_words: {top_words}"
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get word frequency: {str(e)}"
        )
...
async def get_top_users(
    limit: Optional[int] = Query(None, description="Limit number of posts to analyze"),
    top_users: Optional[int] = Query(10, description="Number of top users to return"),
):
    """
    Get top users by unique word count

    Args:
        limit: Optional limit on number of posts to analyze
        top_users: Number of top users to return (default: 10)

    Returns:
        List of UserSummary objects
    """
    try:
        logger.info(f"Getting top users with limit: {limit}, top_users: {top_users}")
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get top users: {str(e)}"
        )
